...
const PurePreviewMessage = ({
  ...
  const parts = message.parts?.map((part, index) => {
      ...
      return (
        <MessageContent
          className={cn("text-[13px] leading-[1.65]", {
            "w-fit max-w-[min(80%,56ch)] overflow-hidden break-words rounded-2xl rounded-br-lg border border-border bg-gradient-to-br from-secondary to-muted px-3.5 py-2 shadow-[var(--shadow-card)]":
              message.role === "user",
          })}
    ...
    if (type === "tool-selectAppointmentType") {
      const { toolCallId, state } = part;
      ...
      if (state === "output-available") {
        return (
          <div className={widthClass} key={toolCallId}>
            <AppointmentTypeSelector data={part.output} />
          </div>
        );
      }
      ...
      return (
        <div className={widthClass} key={toolCallId}>
          <Tool className="w-full" defaultOpen={true}>
            <ToolHeader state={state} type="tool-selectAppointmentType" />
            <ToolContent>
              {state === "input-available" && <ToolInput input={part.input} />}
            </ToolContent>
          </Tool>
